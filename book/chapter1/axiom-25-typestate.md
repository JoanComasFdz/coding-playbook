...
- A transition is a function whose signature is `S₁ → S₂` — the input is the state you're in, the output is the state you're in next. When the transition can fail, the return is `Result<S₂, string>` ([Axiom 14](axiom-14-result.md)).
...
`HasRole` no longer lives on `AuthenticationService`. The only way to obtain an `AuthenticatedSession` is `Authenticate`, which returns a `Result` ([Axiom 14](axiom-14-result.md)). Asking about roles before the result has been unwrapped on the success branch is not a runtime check that returns false — it is a method that does not exist on the value the caller is holding. The silent-false-from-the-bug class is gone, and so is every `if (user != null)` guard that used to defend against it.
...
A function whose parameter type is `AuthenticatedSession` is honest in a way a function whose parameter is `AuthenticationService` cannot be — the signature documents that the function expects an authenticated session. The reader does not have to read the body to discover the precondition; the type carries it. The same property [Axiom 16](axiom-16-value-objects.md) gave to values, applied to call sequences.
...
A long-lived shell ([Axiom 24](axiom-24-stateful-shell.md)) that holds an `AuthenticatedSession` instead of a nullable `User?` field has a smaller surface for the rest of the program to misuse. Stateful resources held in the shell are typed by what they are, not by what they might be — and the shell's seams to the pure core ([Axiom 10](axiom-10-impureheim.md)) carry the same guarantee.
...
[3] **Yaron Minsky**, *Effective ML*, Jane Street technical talks and writing, 2010. Cross-listed from [Axiom 5](axiom-05-honest-total-signatures.md), [Axiom 16](axiom-16-value-objects.md), [Axiom 19](axiom-19-discriminated-unions.md), and [Axiom 20](axiom-20-illegal-states.md). The "make illegal states unrepresentable" slogan as applied to *call sequences* is this axiom; as applied to values is [Axiom 16](axiom-16-value-objects.md); as applied to a record's *shape* is [Axiom 20](axiom-20-illegal-states.md); as applied to outcomes is [Axiom 19](axiom-19-discriminated-unions.md). Slices of the same principle.
