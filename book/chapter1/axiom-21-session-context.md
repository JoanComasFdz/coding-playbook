...
- It is *not* the entity's state ([Axiom 20](axiom-20-state-machines.md)); it is *not* a dependency wired at construction; it is *not* a global. Each of those is a different thing, with its own home.

Every prior axiom has moved in one direction. Data is a value ([Axiom 0](axiom-00-data-vs-behaviour.md)), data does not change after creation ([Axiom 1](axiom-01-immutability.md)), decisions are pure ([Axiom 4](axiom-04-pure-functions.md)), the inputs and outputs of every function are immutable values ([Axioms 5](axiom-05-honest-total-signatures.md), [13](axiom-13-result.md), [18](axiom-18-discriminated-unions.md), [19](axiom-19-pure-functions-returning-actions.md), [20](axiom-20-state-machines.md)). State has existed — the FSM's state in [Axiom 20](axiom-20-state-machines.md) is the most explicit example — but it has always flowed *as an immutable value*: passed in, used to compute the next value, persisted by the shell. Twenty-one axioms of values being created, transformed, and returned. Never mutated. Stateless execution.
...
- **Entity state.** The Subscription's state from [Axiom 20](axiom-20-state-machines.md) lives in the entity, not in the shell's working memory.
...
`ValidateRow` does not take the context, does not see it, cannot mutate it. Its signature is the same one [Axiom 13](axiom-13-result.md) introduced — `string[] -> Result<Customer, string>` — testable as a table with no orchestration setup. The session's accumulator lives one layer out, in the shell, where mutation is allowed.
...
**Entity state.** When the working memory is *the entity's condition* — flags, timestamps, counters that move together because the entity is moving through states — the right home is [Axiom 20](axiom-20-state-machines.md): promote the fields to a State DU and the rules to `Transition`. The Session Context is *not* a shortcut for that refactor; the smells the two axioms address are different.
