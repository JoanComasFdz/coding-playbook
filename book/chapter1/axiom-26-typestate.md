...
- A transition is a function whose signature is `S₁ → S₂` — the input is the state you're in, the output is the state you're in next. When the transition can fail, the return is `Result<S₂, string>` ([Axiom 15](axiom-15-result.md)).
...
`HasRole` no longer lives on `AuthenticationService`. The only way to obtain an `AuthenticatedSession` is `Authenticate`, which returns a `Result` ([Axiom 15](axiom-15-result.md)). Asking about roles before the result has been unwrapped on the success branch is not a runtime check that returns false — it is a method that does not exist on the value the caller is holding. The silent-false-from-the-bug class is gone, and so is every `if (user != null)` guard that used to defend against it.
...
A function whose parameter type is `AuthenticatedSession` is honest in a way a function whose parameter is `AuthenticationService` cannot be — the signature documents that the function expects an authenticated session. The reader does not have to read the body to discover the precondition; the type carries it. The same property [Axiom 17](axiom-17-value-objects.md) gave to values, applied to call sequences: Design by Contract's *precondition*, carried by the type and checked at compile time rather than asserted at runtime.
...
A long-lived shell ([Axiom 25](axiom-25-stateful-shell.md)) that holds an `AuthenticatedSession` instead of a nullable `User?` field has a smaller surface for the rest of the program to misuse. Stateful resources held in the shell are typed by what they are, not by what they might be — and the shell's seams to the pure core ([Axiom 11](axiom-11-impureheim.md)) carry the same guarantee.
...
[3] **Yaron Minsky**, *Effective ML*, Jane Street technical talks and writing, 2010. Cross-listed from [Axiom 5](axiom-05-honest-total-signatures.md), [Axiom 17](axiom-17-value-objects.md), [Axiom 20](axiom-20-discriminated-unions.md), and [Axiom 21](axiom-21-illegal-states.md). The "make illegal states unrepresentable" slogan as applied to *call sequences* is this axiom; as applied to values is [Axiom 17](axiom-17-value-objects.md); as applied to a record's *shape* is [Axiom 21](axiom-21-illegal-states.md); as applied to outcomes is [Axiom 20](axiom-20-discriminated-unions.md). Slices of the same principle.
