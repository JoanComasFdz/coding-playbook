...
A function whose parameter type is `AuthenticatedSession` is honest in a way a function whose parameter is `AuthenticationService` cannot be — the signature documents that the function expects an authenticated session. The reader does not have to read the body to discover the precondition; the type carries it. The same property [Axiom 17](axiom-17-value-objects.md) gave to values, applied to call sequences.
